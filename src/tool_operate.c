                                                ...
                                                uint32_t delayms)
{
  ...
  /* loop through the list to check if we already got this origin */
  item = list;
  do {
  ...
  /* origin was not found so we create a new item and append it to the list */
  new_item = curlx_malloc(sizeof(struct curl_429_list));
  if(!new_item)
    return NULL;
...
/*
 * extract the scheme, host and port from the URL and write it to porigin
 * porigin needs to be freed by the user of this function
 */
static CURLcode origin_from_url(const char *url, char **porigin)
{
  char *host = NULL;
  char *port = NULL;
  char *scheme = NULL;
  ...
  len_port = strlen(port);
  len_scheme = strlen(scheme);
  len_origin = len_host + len_port + len_scheme;
  *porigin = curlx_malloc(len_origin + 3);
  origin = *porigin;
  if(!origin) {
    err = CURLE_OUT_OF_MEMORY;
    goto out;
  }
  memcpy(origin, host, len_host);
  memcpy(origin + len_host + 1, port, len_port);
  memcpy(origin + len_host + len_port + 2, scheme, len_scheme);
  /* add separators between the parts and a 0 termination at the end */
  *(origin + len_host) = '/';
  *(origin + len_host + len_port + 1) = '/';
  *(origin + len_origin + 3) = 0;
                              ...
                              bool *added, bool *skipped)
{
  ...
  while(state->urlnode) {
    ...
    /* extract the origin from the URL for 429 delay checks */
    result = origin_from_url(per->url, &per->origin);
    if(result)
      return result;
